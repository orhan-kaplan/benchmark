"""Chat completions router — /v1/chat/completions endpoint.

Handles chat completion requests by validating input, looking up the
    ...
    return ChatCompletionResponse(
        ...
        choices=[
            ChatCompletionChoice(
                ...
                message=ChatMessage(
                    role="assistant",
                    content="This is a test chat completion response.",
                ),
                finish_reason="stop",
            )
        ],
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if config.test_mode:
        mock = _mock_chat_completion_response(request)
    ...
    # --- Transform response ---
    try:
        chat_completion = _transformer.from_vllm_format(
            vllm_data, request_type="chat_completion"
        )
    except TransformationError as exc:
        return _build_error(500, "transformation_error", exc.message)
