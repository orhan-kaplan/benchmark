...
def _ok_response(content="Hello world!", prompt_tokens=5, completion_tokens=3):
    """Non-streaming OpenAI uyumlu JSON yanıt."""
    return json.dumps({
        "id": "chatcmpl-test",
...
class TestChatCompletion:
    @pytest.mark.asyncio
    async def test_successful_response(self, api_client, default_params, default_messages, httpx_mock):
        httpx_mock.add_response(
            url="http://localhost:8000/v1/chat/completions",
            status_code=200,
            text=_ok_response("Hello world!"),
        )
    ...
    @pytest.mark.asyncio
    async def test_http_error_response(self, api_client, default_params, default_messages, httpx_mock):
        httpx_mock.add_response(
            url="http://localhost:8000/v1/chat/completions",
            status_code=500,
    ...
    @pytest.mark.asyncio
    async def test_retry_on_network_error(self, default_params, default_messages, httpx_mock):
        client = APIClient(timeout=5.0, max_retries=2)
        httpx_mock.add_exception(httpx.ConnectError("Connection refused"))
        httpx_mock.add_response(
            url="http://localhost:8000/v1/chat/completions",
            status_code=200,
            text=_ok_response("OK"),
        )
    ...
    @pytest.mark.asyncio
    async def test_all_retries_exhausted(self, default_params, default_messages, httpx_mock):
        client = APIClient(timeout=5.0, max_retries=2)
        httpx_mock.add_exception(httpx.ConnectError("Connection refused"))
        httpx_mock.add_exception(httpx.ConnectError("Connection refused"))
        result = await client.chat_completion(
    ...
    @pytest.mark.asyncio
    async def test_ngrok_url_support(self, api_client, default_params, default_messages, httpx_mock):
        httpx_mock.add_response(
            url="https://abc123.ngrok-free.app/v1/chat/completions",
            status_code=200,
            text=_ok_response("ngrok works"),
        )
        result = await api_client.chat_completion(
            "https://abc123.ngrok-free.app", "test-model", default_messages, default_params
        )
        assert result.status_code == 200
        assert result.response_text == "ngrok works"

    @pytest.mark.asyncio
    async def test_endpoint_trailing_slash(self, api_client, default_params, default_messages, httpx_mock):
        httpx_mock.add_response(
            url="http://localhost:8000/v1/chat/completions",
            status_code=200,
            text=_ok_response("ok"),
    ...
    @pytest.mark.asyncio
    async def test_empty_choices(self, api_client, default_params, default_messages, httpx_mock):
        httpx_mock.add_response(
            url="http://localhost:8000/v1/chat/completions",
            status_code=200,
            text=json.dumps({"choices": [], "usage": {"prompt_tokens": 5, "completion_tokens": 0, "total_tokens": 5}}),
        )
        result = await api_client.chat_completion(
            "http://localhost:8000", "test-model", default_messages, default_params
        )
    ...
    @pytest.mark.asyncio
    async def test_request_payload_format(self, api_client, default_messages, httpx_mock):
        ...
        await api_client.chat_completion("http://localhost:8000", "my-model", default_messages, params)
